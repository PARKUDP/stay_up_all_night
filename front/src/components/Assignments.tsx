...
const Assignments: React.FC = () => {
        ...
        .then(async (response: AxiosResponse<{ class_name: string; assignments: Assignment[] }>) => {
            ...
            const assignmentsWithCount = await Promise.all(
                response.data.assignments.map(async (assignment) => {
                    const statusResponse = await axios.get(`http://127.0.0.1:5000/assignments/${assignment.id}/status`, {
                        headers: { Authorization: `Bearer ${token}` }
                    });
                    const completionResponse = await axios.get(`http://127.0.0.1:5000/assignments/${assignment.id}/completion_count`, {
                        headers: { Authorization: `Bearer ${token}` }
                    });
                    return { 
                        ...assignment, 
    ...
    const updateStatus = (assignmentId: number, newStatus: '未着手' | '進行中' | '完了') => {
        ...
        axios.put(`http://127.0.0.1:5000/assignments/${assignmentId}/status`, 
            { status: newStatus }, 
            {
                headers: { Authorization: `Bearer ${token}` }
            }
    ...
    const deleteAssignment = (id: number) => {
        ...
        const token = localStorage.getItem('token');
        axios.delete(`http://127.0.0.1:5000/assignments/${id}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
